...
const nextConfig: NextConfig = {
  ...
  outputFileTracingIncludes: {
    // Applies to every route, not just the four obviously-dynamic ones: the
    // statically generated pages carry `revalidate`, so their `.rsc` requests
    // re-render on the server and need the database too. Production was
    // erroring on `/`, `/news/<slug>` and `/events/<slug>` as well.
    "/**": [
      // Generated by `npm run prebuild` and gitignored, so it never arrives
      // via the repo; opened through a runtime path in src/lib/db/pool.ts,
      // which the tracer cannot follow.
      "./data/sgsm.db",
      // node-sqlite3-wasm resolves its .wasm binary at runtime rather than
      // through a static import, so Next's tracer never sees it and left it
      // out of the bundle. Production failed with
      // "ENOENT: /var/task/node_modules/node-sqlite3-wasm/dist/node-sqlite3-wasm.wasm".
